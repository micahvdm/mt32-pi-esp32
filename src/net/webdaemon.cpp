...
THTTPStatus CWebDaemon::BuildSoundPage(u8* pBuffer, unsigned* pLength, const char** ppContentType)
{
		...
		html.Append("const applyActiveSynth=async(value)=>{const lbl=document.getElementById('rt_active_synth_label');const current=lbl?lbl.textContent:'';if((value==='mt32'&&current==='MT-32')||(value==='soundfont'&&current==='SoundFont')||(value==='opl3'&&current==='OPL3')){setActiveSynthUI(value);if(rs)rs.textContent='';return;}setActiveSynthUI(value);const ok=await rtApply('active_synth',value,false);await rtRefresh();if(!ok){if(rs)rs.textContent='Could not apply active_synth';showToast('Error: active_synth',false);}};");
