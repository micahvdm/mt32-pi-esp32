...
THTTPStatus CWebDaemon::BuildMixerPage(u8* pBuffer, unsigned* pLength, const char** ppContentType)
{
		...
		html.Append("eb.textContent=c.engine==='MT-32'?'MT-32':(c.engine==='OPL3'?'OPL':'SF');eb.dataset.ch=c.ch;eb.dataset.eng=c.engine==='MT-32'?'mt32':(c.engine==='OPL3'?'opl3':'fluidsynth');");
